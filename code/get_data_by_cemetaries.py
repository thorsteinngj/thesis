...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 25 15:25:28 2019
...
for num in range(1,100):
    cemetery = 'https://www.findagrave.com/cemetery/63913/memorial-search?page='+str(num)
    ua = random_headers()
    req = Request(cemetery, headers=ua)
    page_raw = urlopen(req).read()
    #HTML parsing
    page_soup = soup(page_raw, "html5lib")
        
    
    picture = page_soup.findAll("input",{"title":"Memorial ID"})
    for j in range(len(picture)):
        grave_ids.append(picture[j]['value'])
        
#%%

ids = np.unique(grave_ids)
...
my_dataset_df.to_csv('/home/thorsteinngj/Documents/Skoli/Thesis/Code/jewish_dataset.csv')
